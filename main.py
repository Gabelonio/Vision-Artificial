...
def main():
    ...
    ## kernel segunda derivada
    # vertical [[1], [-2], [1]]
    # horizontal [[1, -2, 1]]

    while True:
        print("\nEscoga el metodo:")
        opt = int(input("1. Segunda derivada\n2. Gradiente\n3. Salir\nOpción: "))
        if opt==1:
            filterVertical = np.array([[1], [-2], [1]])
            filterHorizontal = np.array ([[1, -2, 1]])
            print("\nEscoga la imagen:")
...
def calc(img,gray,filterHorizontal,filterVertical, opt):
    ...
    Analisis(horizontalBorders,verticalBorders)
...
def Analisis(horizontalBorders, verticalBorders):
    plt.figure(3)
    for z in range(1,26):
        yhorizontalBorders, xhorizontalBorders = np.shape(horizontalBorders)
        result = np.ones(np.shape(horizontalBorders))
        ...
        for i in range(yhorizontalBorders):
            for j in range(xhorizontalBorders):
                ...
                if result[i][j] < umbral:
                    ...
                    result[i][j] = 255
        title="Umbral "+str(umbral)
        plt.subplot(5,5,z),plt.imshow(result, cmap='gray'),plt.title(title)
